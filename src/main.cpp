...
#if defined(NDEBUG)
# error "Capitalcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9447bb0ad49b0c4b9756803648e16072be1eeecb98a971fa09c109fad88f0e78");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Capitalcoin: starting difficulty is 1 / 2^12
...
{
    // Capitalcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Capitalcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Capitalcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Capitalcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Capitalcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Capitalcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9447bb0ad49b0c4b9756803648e16072be1eeecb98a971fa09c109fad88f0e78");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "$44 BILLION DOLLAR TECH DEAL USA CHINA";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2147000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0445d97b1d9cd6284056f14b23bfdb6d3ef1e32f3b59b37bf703157337d414a82e1505874120637ad40f52519fd9e5e9ef59b7d3cca1093095b2fe763d8bfc390f") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0xe8336fc4bbb876e36a9d3b99d71aaf9a6cbacfe36a40e06269c6e3f2083c7fd1"));
...
unsigned char pchMessageStart[4] = { 0xfA, 0xd3, 0xa4, 0xbd }; // Capitalcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CapitalcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CapitalcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CapitalcoinMiner(CWallet *pwallet)
{
    printf("CapitalcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CapitalcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CapitalcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CapitalcoinMiner, pwallet));
}
